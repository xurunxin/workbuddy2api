...
func main() {
	...
	srv := &http.Server{
		...
		// 上限由 server.max_body_mb 的 413 预拦截兜住，超大 body 不会被静默喂给上游。
	...
	log.Printf("workbuddy2api listening on %s (api_key_required=%v, max_body_mb=%d)", cfg.Listen, keys.Required(), cfg.Server.MaxBodyMB)
