...
// TestChatLargeBodyProceeds 请求体在上限内正常透传：合法大 body（低于 20MB 默认上限）
// 完整读入并正常进入后续处理（打到上游）。超过 server.max_body_mb 上限时由 413 预拦截
// 拒绝（issue #41：截断 body 会让上游 unmarshal 报 unexpected EOF，网关却罚号轮空）。
func TestChatLargeBodyProceeds(t *testing.T) {
	...
	// 接近但低于 20MB 默认上限的合法 JSON body：必须完整读入、照常进上游。
	pad := strings.Repeat("a", 18<<20)
	var b bytes.Buffer
	...
	if rec.Code != 200 {
		t.Fatalf("code=%d body=%s (body within limit must proceed to upstream)", rec.Code, rec.Body)
	}
	if calls != 1 {
...
// TestChatOversizedBody413 超过 max_body_mb 上限 → 413 request_body_too_large：
// 不打上游、不罚账号、不轮转（413 是网关侧的客户端问题）。
func TestChatOversizedBody413(t *testing.T) {
	...
	p := testPoolWith(&auth.Auth{UID: "u1", AccessToken: "at1", ExpiresAt: 9999999999})
	h := NewHandler(Config{Pool: p, Upstream: up})
	// 超过 20MB 默认上限的 body（22MB）：必须 413，且不触达上游。
	pad := strings.Repeat("a", 22<<20)
	var b bytes.Buffer
	b.WriteString(`{"model":"glm-5.2","messages":[],"pad":"`)
	b.WriteString(pad)
	b.WriteString(`"}`)
	...
	assertJSONErrorCode(t, rec.Body.String(), "request_body_too_large")
	if calls != 0 {
...
func TestModelsDynamic(t *testing.T) {
	...
	// 第二次调用仍返回相同动态名单（纯动态缓存语义，不再有 source 字段）。
	rec = httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest("GET", "/v1/models", nil))
	if rec.Code != 200 {
		t.Fatalf("second call code=%d", rec.Code)
	}
}
