...
type Config struct {
	...
	// MaxBodyBytes 聊天请求体大小上限；<=0 兜底 20<<20（20MB）。
...
func (h *Handler) withAuth(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 静态 key 与 keys 体系（IdentifyAPIKey/ValidateAPIKey）都走常量时间比较。
		staticValid := h.cfg.APIKey == "" ||
			subtle.ConstantTimeCompare([]byte(key), []byte(h.cfg.APIKey)) == 1
		valid := staticValid
		keyID := "anonymous"
		if h.cfg.APIKey != "" {
			keyID = "legacy"
...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < h.cfg.MaxRotate; i++ {
		...
		// sErr 提升到轮转循环作用域：defer 在函数出口读取（流式写失败/空流 → u.failed）。
		var sErr error
		defer func() {
			if u, ok := r.Context().Value(usageContextKey{}).(*requestUsage); ok {
				u.input, u.output, u.known = stats.PromptInput(), int64(stats.tokens), stats.hasUsage
				u.cached = stats.CachedTokens()
				u.failed = upstream.IsEmptyStreamError(sErr)
			}
		}()
		if peek.Stream {
