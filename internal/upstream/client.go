...
// FetchModelsContext is FetchModels with a caller context (catalog/admin 面板
// 需要请求级取消；chat 主路径走 FetchModels 即 Background）。
// 实现为对两路并发探测的同步等待：ctx 取消时 goroutine 自然结束（channel 缓冲 1）。
...
// branches used by UserResourceDetailed. Used is nil when the upstream response
// does not provide enough fields to determine current-cycle usage; nil must
// not be interpreted as zero usage.（本分支 admin 面板「查询积分」动作的口径。）
func (c *Client) ResourceUsage(a *auth.Auth) (usage ResourceUsage, err error) {
	// 指针形态解析：需要区分「上游未下发 used 字段」（used 未知）与「显式 0」，
	// 共享的 plain-int64 userResourceResp 做不到，这里单独解析原始 JSON。
	body := map[string]any{
		"PageNumber":               1,
		"PageSize":                 100,
		"ProductCode":              "p_tcaca",
		"Status":                   []int{0, 3},
		"PackageEndTimeRangeBegin": time.Now().Format(packageEndLayout),
		"PackageEndTimeRangeEnd":   time.Now().Add(365 * 101 * 24 * time.Hour).Format(packageEndLayout),
	}
	...
	var resp struct {
		Response struct {
			Data struct {
				Accounts []resourcePackagePtr `json:"Accounts"`
			} `json:"Data"`
		} `json:"Response"`
	}
	...
	for _, acct := range resp.Response.Data.Accounts {
		usage.Remain += resourceRemainPtr(acct)
		if value, ok := resourceUsedPtr(acct); ok {
			used += value
		} else {
			usedKnown = false
		}
...
// ResourceUsage 余额/已用快照（admin 面板展示；Used 缺失时为 nil ≠ 0）。
type ResourceUsage struct {
	Remain int64
	Used   *int64
}

// resourcePackagePtr get-user-resource 的单套餐形态（指针字段区分「上游未下发」
// 与「显式 0」——ResourceUsage 的 used 未知性判断依赖这一点，不能用 plain int64）。
type resourcePackagePtr struct {
...
// resourceRemainPtr applies the historical balance-selection precedence:
// current-cycle capacity when present, current-cycle fields when populated,
// then lifetime/package capacity.
func resourceRemainPtr(a resourcePackagePtr) int64 {
	var remain int64
	switch {
...
// resourceUsedPtr follows the same branch as resourceRemainPtr. An explicit used
...
func resourceUsedPtr(a resourcePackagePtr) (int64, bool) {
	var used *int64
